...
<body>
    ...
    <script type="text/javascript">
        ...
        (function($) {
            // 从后端变量获取模型最终地址
           var finalOfdUrl = '${finalUrl}';
		   var kkagent = '${kkagent}';
		   var baseUrl = '${baseUrl}'.endsWith('/') ? '${baseUrl}' : '${baseUrl}' + '/';
          if (kkagent === 'true' || !finalOfdUrl.startsWith(baseUrl)) {
           finalOfdUrl = baseUrl + 'getCorsFile?urlPath=' + encodeURIComponent(Base64.encode(finalOfdUrl))+ "&key=${kkkey}" + "&fullfilename=/${file.name}";
      }
            ...
            $(window).on('load', function() {
                // 等待查看器核心初始化完成后再加载模型
                setTimeout(() => {
                    if (finalOfdUrl && finalOfdUrl !== '') {
                        loadModelFromUrl(finalOfdUrl);
                    } else {
